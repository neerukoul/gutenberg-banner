	...
	edit: props => {
		const {
			attributes: {
				...
				mediaURL,
				mediaALT,
				mediaCaption,content, nodeName,align,color,hasImage,
			},
			focus,
			setFocus,
			setAttributes,
		...
		return (
			<div className={ className + ' container-wrapper alignwide' }>
				{
					!! focus && !! mediaId && (
		...
		return (
			<div className={ className + ' container-wrapper alignwide' }>
				...
				<div className="info-container">
					...
					{ bannerLink ? <a href={ bannerLink } className="container-link button">{ __( 'Read More' ) }</a> : '' }
				</div>
			</div>
		);
	},
